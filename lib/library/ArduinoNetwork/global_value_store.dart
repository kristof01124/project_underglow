...
class GVSUpdateRecordData extends PairMessage<MessageUint16, MessageFloat32> {
  GVSUpdateRecordData(MessageUint16 first, MessageFloat32 second) : super(first, second);
}

class GvsUpdate extends NetworkMessage<ListMessage> {
    GvsUpdate() : super(MessageHeader(
      NetworkManager.protocol,
              GlobalValueStore.globalValueStoreIp,
              GlobalValueStore.globalValueStoreIp,
              const MessageType(
        ...
        (int size) => List.filled(size, GVSUpdateRecordData(MessageUint16(0), MessageFloat32(0))),
    ));
}
...
class GlobalValueStore extends NetworkEntity{
    ...
    header.build(buffer);
    if (header.messageType == const MessageType(messagePrimaryType, updateMessageSecondaryType)) {
